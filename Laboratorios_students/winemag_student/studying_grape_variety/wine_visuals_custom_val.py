...
def generate_wordcloud(wines_in_cluster, level, category=None, color=None, title=None):
    ...
    # Use only the 'descriptors_level_%s'%level that are from the variable category of descriptor_level_1 (ex body) to make the wordcloud 
    if category is None:
...
def generate_cluster_visual(filtered_dataframe, full_dataframe, title, categories = ['fruit', 'oak', 'acid', 'body']):
    ...
    generate_wordcloud(wines_in_cluster=filtered_dataframe, level='descriptor_level_3', category=categories[0], color='#223344')

    plt.subplot2grid(subplot_grid, (8, 0), rowspan=4)
    generate_wordcloud(wines_in_cluster=filtered_dataframe, level='descriptor_level_3', category=categories[1], color='#916e31')

    plt.subplot2grid(subplot_grid, (12, 0), rowspan=4)
    generate_wordcloud(wines_in_cluster=filtered_dataframe, level='descriptor_level_3', category=categories[2], color='#c88c27')

    plt.subplot2grid(subplot_grid, (16, 0), rowspan=4)
    generate_wordcloud(wines_in_cluster=filtered_dataframe, level='descriptor_level_3', category=categories[3], color='#581845')
